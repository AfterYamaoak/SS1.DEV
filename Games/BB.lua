...
if game.PlaceVersion > 1289 then
    local Loaded,PromptLib = false,loadstring(game:HttpGet("https://raw.githubusercontent.com/AfterYamaoak/SS1.DEV/main/ROBLOX/PromptLibrary.lua"))()
    PromptLib("Unsupported game version","You are at risk of getting autoban\nAre you sure you want to load SS1.DEV?",{
...
local Window = SS1.Utilities.UI:Window({
    Name = "SS1.DEV — "..SS1.Game,
    ...
    local SettingsTab = Window:Tab({Name = "Settings"}) do
        ...
        Callback = SS1.Utilities.Misc.ReJoin})
        SettingsTab:Button({Name = "Server Hop",Side = "Left",
        Callback = SS1.Utilities.Misc.ServerHop})
        SettingsTab:Button({Name = "Join Discord Server",Side = "Left",
        Callback = SS1.Utilities.Misc.JoinDiscord})
        ...
        local CreditsSection = SettingsTab:Section({Name = "Credits",Side = "Right"}) do
            CreditsSection:Label({Text = "This script was made by SS1"})
...
SS1.Utilities.Misc:SetupWatermark(Window)
SS1.Utilities.Drawing:SetupCursor(Window.Flags)

SS1.Utilities.Drawing:FOVCircle("Aimbot",Window.Flags)
SS1.Utilities.Drawing:FOVCircle("Trigger",Window.Flags)
SS1.Utilities.Drawing:FOVCircle("SilentAim",Window.Flags)
...
Notify.Event:Connect(function(Text)
    SS1.Utilities.UI:Notification2(Text)
end)
    ...
    if Weapon and Config then
        ...
        if Ammo.Value > 0 then
            ...
            if (OldAmmo - Ammo.Value) >= 1 then
                SS1.Utilities.UI:Notification2({
...
SS1.Utilities.Misc:CreateHook(Tortoiseshell.Network.Fire,function(Self,...)
...
SS1.Utilities.Misc:CreateHook(Tortoiseshell.Projectiles.InitProjectile,function(Self,...)
...
SS1.Utilities.Misc:CreateHook(Tortoiseshell.Raycast.CastGeometryAndEnemies,function(Self,...)
...
SS1.Utilities.Misc:CreateHook2(Tortoiseshell.Items.GetConfig,function(...)
...
--[[for Index,Event in pairs(Events) do
    if Event.Event == "Votekick" then
        OldCallback = SS1.Utilities.Misc:CreateHook(Event.Callback,function(...)
...
SS1.Utilities.Misc:NewThreadLoop(0,function()
...
SS1.Utilities.Misc:NewThreadLoop(1,function()
    local Weapon,Config = GetEquippedWeapon()
    if Weapon and Config then
...
for Index,Player in pairs(PlayerService:GetPlayers()) do
    ...
    SS1.Utilities.Drawing:AddESP(Player,"Player","ESP/Player",Window.Flags)
end
PlayerService.PlayerAdded:Connect(function(Player)
    SS1.Utilities.Drawing:AddESP(Player,"Player","ESP/Player",Window.Flags)
end)
PlayerService.PlayerRemoving:Connect(function(Player)
    SS1.Utilities.Drawing:RemoveESP(Player)
end)
