...
local function GetConfigs()
    if not isfolder("SS1") then makefolder("SS1") end
    if not isfolder("SS1\\Configs") then makefolder("SS1\\Configs") end
    if not isfile("SS1\\DefaultConfig.txt") then writefile("SS1\\DefaultConfig.txt","") end

    local Configs = {}
    for Index,File in pairs(listfiles("SS1\\Configs") or {}) do
        File = File:gsub("SS1\\Configs\\","")
        File = File:gsub(".json","")
        Configs[Index] = File
    end
    return Configs
...
local function ConfigsToList()
    ...
    for Index,File in pairs(listfiles("SS1\\Configs") or {}) do
        File = File:gsub("SS1\\Configs\\","")
        File = File:gsub(".json","")
        local DefaultConfig = readfile("SS1\\DefaultConfig.txt")
...
local function InitScreen()
    ...
    --[[if Debug then
        ...
        SS1.Utilities.Misc:HideObject(ScreenAsset)
    end]]
...
local function InitWindow(ScreenAsset,Window)
    ...
    function Window:SaveConfig(Name)
        ...
        if table.find(GetConfigs(),Name) then
            Config = HttpService:JSONDecode(readfile("SS1\\Configs\\"..Name..".json"))
        end
        for Index,Element in pairs(Window.Elements) do
        ...
        writefile("SS1\\Configs\\"..Name..".json",HttpService:JSONEncode(Config))
    end
    function Window:LoadConfig(Name)
        if table.find(GetConfigs(),Name) then
            local DecodedJSON = HttpService:JSONDecode(readfile("SS1\\Configs\\"..Name..".json"))
            for Index,Element in pairs(Window.Elements) do
            ...
            local ConfigSection = Tab:Section({Name = "Configs",Side = Side}) do
                ...
                ConfigSection:Button({Name = "Set",Callback = function()
                    if ConfigDropdown.Value and ConfigDropdown.Value[1] then
                        ...
                        writefile("SS1\\DefaultConfig.txt",DefaultConfig)
                        ConfigDivider:SetText(
                        "Default Config\n<font color=\"rgb(189,189,189)\">[ "..DefaultConfig.." ]</font>")
                    end
                end})
                ConfigSection:Button({Name = "Clear",Callback = function()
                    writefile("SS1\\DefaultConfig.txt","")
